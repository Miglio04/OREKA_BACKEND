...
import re
from models import POSLine
from kpi_calculations import kpi_pos_only
...
class FileProcessor:
    ...
    def _extract_invoice_info(self, text: str) -> Dict[str, Any]:
        ...
        for field, pattern in patterns.items():
            ...
            if matches:
                invoice_info[field] = matches[0] if isinstance(matches[0], str) else matches[0]
    ...
    def get_computations_summary(self) -> Dict[str, Any]:
        ...
        if pos_lines:
            ...
            summary.update({
                ...
                'revenue_by_area': {k: float(v) for k, v in kpi_results.get('revenue_by_area', {}).items()},
                'revenue_by_payment': {k: float(v) for k, v in kpi_results.get('revenue_by_payment', {}).items()},
                'receipt_count': kpi_results.get('receipt_count', 0),
                'average_receipt': float(kpi_results.get('average_receipt', 0)),
                'discount_rate': float(kpi_results['discount_rate']) if kpi_results.get('discount_rate') is not None else None
            })
        ...
        summary['recent_files'] = [
            {
                ...
                'records': f.get('total_records') if f.get('file_type') == 'csv' else f.get('page_count')
            }
            for f in files[:10]
        ]
